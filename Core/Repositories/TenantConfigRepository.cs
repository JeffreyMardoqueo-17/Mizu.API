...
public sealed class TenantConfigRepository : RepositoryBase, ITenantConfigRepository
{
    ...
    {
        const string sql = """
                   INSERT INTO tenant_configs (id, tenant_id, moneda, limite_consumo_fijo, precio_consumo_fijo, limite_consumo_extra1, cargo_extra1, limite_consumo_extra2, cargo_extra2, limite_consumo_extra3, cargo_extra3, cargo_exceso_mayor, tramos_consumo_json, multa_retraso, multa_no_asistir_reunion, multa_no_asistir_trabajo, permitir_multiples_contadores, maximo_contadores_por_usuario)
                   VALUES (@Id, @TenantId, @Moneda, @LimiteConsumoFijo, @PrecioConsumoFijo, @LimiteConsumoExtra1, @CargoExtra1, @LimiteConsumoExtra2, @CargoExtra2, @LimiteConsumoExtra3, @CargoExtra3, @CargoExcesoMayor, CAST(@TramosConsumoJson AS jsonb), @MultaRetraso, @MultaNoAsistirReunion, @MultaNoAsistirTrabajo, @PermitirMultiplesContadores, @MaximoContadoresPorUsuario)
                   """;
    ...
    {
        const string sql = """
                   SELECT id, tenant_id, moneda, limite_consumo_fijo, precio_consumo_fijo, limite_consumo_extra1, cargo_extra1, limite_consumo_extra2, cargo_extra2, limite_consumo_extra3, cargo_extra3, cargo_exceso_mayor, tramos_consumo_json AS TramosConsumoJson, multa_retraso, multa_no_asistir_reunion, multa_no_asistir_trabajo, permitir_multiples_contadores AS PermitirMultiplesContadores, maximo_contadores_por_usuario AS MaximoContadoresPorUsuario
                   FROM tenant_configs
    ...
    public Task<bool> ActualizarAsync(TenantConfig config, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        const string sql = """
                           ...
                           SET moneda = @Moneda,
                               ...
                               permitir_multiples_contadores = @PermitirMultiplesContadores,
                               maximo_contadores_por_usuario = @MaximoContadoresPorUsuario
                           WHERE tenant_id = @TenantId
                           """;
